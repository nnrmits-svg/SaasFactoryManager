-- Sprint A: Tablas para el Help Center / AI Fluya
-- Aplicar en proyecto Supabase SaasFactoryManager (ref: fxlvexilnrfkkcbzwskr)
--
-- Backing store del chatbot AI Fluya. Lectura publica (is_active/is_published),
-- escritura solo desde admin via service_role (no hay UI admin todavia, se popula
-- desde un seed script o manualmente).
...
DROP POLICY IF EXISTS "categories readable by all" ON public.help_categories;
DROP POLICY IF EXISTS "articles readable by all" ON public.help_articles;
DROP POLICY IF EXISTS "faqs readable by all" ON public.faqs;
DROP POLICY IF EXISTS "feedback readable by owner" ON public.article_feedback;
DROP POLICY IF EXISTS "feedback insertable by authenticated" ON public.article_feedback;
...
DROP TRIGGER IF EXISTS trg_help_categories_updated ON public.help_categories;
